...
const JobDetailsSummary = ({ onEdit }: JobDetailsSummaryProps) => {
  ...
  const formatSalary = (val?: string | number) => {
    ...
    return isNaN(num) ? undefined : num.toLocaleString("en-GB");
  };

  const fromFmt = formatSalary(values.salaryFrom);
  const toFmt = formatSalary(values.salaryTo);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Grid: Role | Employment type, Location | Salary, Deadline */}
      <div className="grid grid-cols-2 gap-x-6 gap-y-5">
        <SummaryRow label="Role" value={ROLES.find((r) => r.value === values.role)?.label} />
      ...
      {values.description && (
        <div className="flex flex-col gap-1">
          <p className="text-sm text-neutral-400">Job description</p>
          <p className="text-base text-neutral-700 wrap-break-word">{values.description}</p>
        </div>
      )}

      {/* Business problem */}
      {values.businessProblem && (
        <div className="flex flex-col gap-1">
          <p className="text-sm text-neutral-400">Business problem to solve</p>
