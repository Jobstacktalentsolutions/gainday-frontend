...
export const jobDetailsBaseSchema = z.object({
    ...
    salaryFrom: z.preprocess(
        ...
        z.coerce.number().int("Must be a whole number").positive("Must be greater than 0").optional()
    ),
    ...
    // pipeline's "Problem" extraction separately from `description`.
...
export const jobDetailsSchema = jobDetailsBaseSchema.refine(
    (data) => {
        const from = parseNum(data.salaryFrom);
        const to = parseNum(data.salaryTo);
